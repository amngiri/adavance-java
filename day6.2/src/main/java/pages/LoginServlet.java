	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			// not null => successful login : send validated user details to clnt
				...
				pw.print("login successful : from login page");
				pw.flush();
				// which min scope to choose in SP : to store user details : request
				...
				// WC suspends exec of the current page(method) --retains PW's buffer --WC
				// invokes --doPost method of the topics page -- after it's exec over --control
				// comes back to login page.
				//  main page n included page in the chain(Login n Topics page) can generate the dyn resp)
				System.out.println("control of exec came back to login....");
				pw.print("<h5> login page sending some contents post include....</h5>");
			}
		} catch (Exception e) {
